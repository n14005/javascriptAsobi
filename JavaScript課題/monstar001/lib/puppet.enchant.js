...
enchant.puppet = {
    assets: [
        ...
        'chara5.png', 'chara6.png', 'chara7.png', 'enemy01.png',
        'effect0.gif', 'eclipse.png', 'hollywood.png',
        'desert.png', 'beach.png', 'sky.png', 'spacebg.png',
        'table.png', 'rpg.png', 'race.png', 'black.png',
        'blockg.png', 'actiong.png', 'map2.png'
    ]
};
...
/**
 * @scope enchant.puppet.Shape.prototype
 */
enchant.puppet.Shape = enchant.puppet.Actor.create(enchant.Sprite, {
    /**
     * @name enchant.puppet.Shape
     * @class
     [lang:ja]
     * 図形オブジェクト.
     * Behavior(振る舞い)を追加することでゲームのための動作を設定することができる.
     * @param {Number} _x 初期位置のx座標.
     * @param {Number} _y 初期位置のy座標.
     [/lang]
     * @constructs
    ...
    initialize: function() {
        ...
        switch (this.type) {
            case 'line':
                this.ox = Math.min(this.sx, this.ex);
                this.oy = Math.min(this.sy, this.ey);
            ...
            case 'circle':
                width = height = (this.r + (this.lineWidth >> 1)) * 2 + 1;
                break;

            case 'filledcircle':
                width = height = this.r * 2 + 1;
                break;
        }
         ...
         * @type String
         */
        this.color = enchant.puppet.Shape.DEFAULT_COLOR;
    },
    cvsRender: function(ctx) {
        ...
        switch (this.type) {
            case 'line':
                ctx.beginPath();
                ctx.moveTo(this.sx - this.ox, this.sy - this.oy);
                ctx.lineTo(this.ex - this.ox, this.ey - this.oy);
                ctx.stroke();
                break;

            case 'rect':
                thick = this.lineWidth >> 1;
                thick2 = thick << 1;
                ctx.strokeRect(thick, thick, this.width - thick2, this.height - thick2);
                break;
            ...
            case 'circle':
                r = this.r + (this.lineWidth >> 1);
                ctx.beginPath();
                ctx.arc(r, r, this.r, 0, Math.PI * 2);
                ctx.stroke();
                break;
...
enchant.puppet.Shape.DEFAULT_COLOR = '#000000';
...
enchant.puppet.Theatre.create = function(option) {
    var autoStart = true;
    var showScore = true;
    var w, h, bg, debug, assets, onload;
    w = h = 320;
    if (option) {
        ...
        onload = (typeof option.onload === 'function') ? option.onload : function() {};
        debug = option.debug;
    }
    var func = function() {
